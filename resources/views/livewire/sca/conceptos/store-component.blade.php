            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content text-bg-dark" >
                    <div class="modal-header">
                        <h3 class="text-xl text-center w-full font-bold text-gray-900">{{ $concepto_id ? 'Editar' : 'Crear' }} Conceptos</h3>
                    </div>
                            ...
                            <div class="text-lg w-full text-black border-2 border-solid border-gray-300">
                                <div class="grid 2xl:grid-cols-5 md:grid-cols-2 sm:grid-cols-2 grid-cols-2 items-center gap-2 mx-2 mt-2" >
                                    <div>
                                        ...
                                        <x-input class="w-full" type="text" wire:model.live="codigo" required id="codigo" name="codigo"/>
                                        @error('codigo')
                                    ...
                                    <div>
                                        <x-label for="accion" value="Operacion:" />
                                        <x-input.select wire:model.live="accion" class="w-full" required id="accion" name="accion" >
                                            <option value=""><-Seleccione-></option>
                                            <option value="+" {{ $accion == '+' ? 'selected' : '' }}>Suma</option>
                                            <option value="-" {{ $accion == '-' ? 'selected' : '' }}>Resta</option>
                                            <option value="*" {{ $accion == '*' ? 'selected' : '' }}>Multiplica</option>
                                            <option value="/" {{ $accion == '/' ? 'selected' : '' }}>Divide</option>
                                        </x-input.select>
                                        @error('accion')
                                            <span class="text-red-500">{{ $message }}</span>
                                        @enderror
                                ...
                                <div class="flex justify-around items-center flex-col-reverse lg:flex-row mb-4 gap-2 mt-4">
                                    ...
                                    <button type="submit"  id="myButton"  class="rounded-md bg-blue-400 text-white hover:bg-blue-800 lg:w-1/3 w-full h-10 text-lg">{{ $concepto_id ? 'Editar' : 'Crear' }}</button>
                                </div>
                        </form>
                    </div>
                </div>
