...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::connection(name: 'sca')->create(table: 'historico_fichas_socios', callback: function (Blueprint $table) {
            ...
            $table->foreignId(column: 'sede_id')->nullable()->references(column: 'id')->on(table: 'uneg.sedes')->onUpdate(action: 'cascade')->onDelete(action: 'restrict');
            $table->foreignId(column: 'zona_id')->nullable()->references(column: 'id')->on(table: 'uneg.zonas')->onUpdate(action: 'cascade')->onDelete(action: 'restrict');
            $table->timestamps();
