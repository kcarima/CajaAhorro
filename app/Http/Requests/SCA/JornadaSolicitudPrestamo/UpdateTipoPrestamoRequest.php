...
final class UpdateTipoPrestamoRequest extends FormRequest
{
    /**
    ...
    {
        $tipo = TipoPrestamo::where('uuid', $this->tipo)->firstOrFail();

        return [
            'nombre' => ['required', Rule::unique('sca.tipos_prestamos', 'nombre')->ignore($tipo)],
            'codigo' => ['required', Rule::unique('sca.tipos_prestamos', 'codigo')->ignore($tipo)],
